...
Connect Cloud rebuilds the environment from a `manifest.json` (not your local library, not `renv.lock`) and runs the R code at build time — so every package the deck loads must be in the manifest. Generate it once, in a **fresh R session** with this folder as the working directory:

```r
source("deploy.R")   # installs all packages (incl. rlang 1.3.0), then writes manifest.json
```

Then commit this whole folder — **including `manifest.json`** — to a GitHub repo and publish it from Connect Cloud (`connect.posit.cloud`).
